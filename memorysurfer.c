...
static enum Action action_seq[S_END+1][14] = {
  ...
  { A_GATHER, A_OPEN, A_READ_PASSWD, A_AUTH_TOK, A_GEN_TOK, A_TEST_CAT, A_SELECT_DEST_DECK, A_END }, // S_SELECT_DEST_CAT
  ...
  { A_GATHER, A_OPEN, A_READ_PASSWD, A_AUTH_TOK, A_GEN_TOK, A_RETRIEVE_MTIME, A_LOAD_CARDLIST, A_CHECK_RESUME, A_UPDATE_QA, A_UPDATE_HTML, A_QUESTION, A_SYNC, A_END }, // S_QUESTION_SYNCED
  { A_GATHER, A_OPEN, A_READ_PASSWD, A_AUTH_TOK, A_GEN_TOK, A_LOAD_CARDLIST, A_CHECK_RESUME, A_QUESTION, A_END }, // S_QUESTION
...
static int gen_html_cat(int16_t n_create, int indent_n, enum HIERARCHY hierarchy, struct WebMemorySurfer *wms) {
  int e;
  int rv;
  char *c_str;
  char indent_str[32];
    ...
    while ((bl = block_seq[wms->page][bl_i++]) != B_END && e == 0)
    {
      ...
      case B_CAT_NAME:
        ...
        if (wms->seq == S_SELECT_CREATE_CAT || wms->seq == S_CAT_NAME) {
          ...
          header_str = "Enter the name to rename the category to.";
      ...
      case B_TABLE:
        ...
        for (i = 0; i < 21 && e == 0; i++) {
          set_time_str(time_str, lvl_s[i]);
          rv = printf("\t\t\t\t<tr><td class=\"msf_histogram\">%d</td><td class=\"msf_histogram\"><code>(%s)</code></td><td class=\"msf_histogram\">%d</td></tr>\n", i, time_str, wms->lvl_bucket[0][i]);
          e = rv < 0;
        }
        if (e == 0) {
          rv = printf("\t\t\t</table>\n"
