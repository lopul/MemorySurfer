    ...
    case 5:
      if (memcmp(mult->post_lp, "Learn", 5) == 0) {
        if (wms->from_page == P_EDIT) {
          ...
          e = wms->from_page != P_SELECT_DECK && wms->from_page != P_SEARCH && wms->from_page != P_HISTOGRAM && wms->from_page != P_PREVIEW;
          if (e == 0) {
            wms->seq = S_QUESTION;
          }
        }
  ...
  if (e == 0) {
    ...
    while ((bl = block_seq[wms->page][bl_i++]) != B_END && e == 0) {
      ...
      case B_PREFERENCES:
        ...
        break;
      case B_ABOUT:
        rv = printf("\t\t\t<h1 class=\"msf\">About MemorySurfer v1.0.1.119</h1>\n"
      ...
      case B_TABLE:
        rv = printf("\t\t\t<h1 class=\"msf\">Table</h1>\n"
                    "\t\t\t<table>\n"
                    "\t\t\t\t<thead>\n"
                    "\t\t\t\t\t<tr><td>Level</td><td>Strength</td><td>Cards</td><td>Threshold</td></tr>\n"
                    "\t\t\t\t</thead>\n"
                    "\t\t\t\t<tbody>\n");
        e = rv < 0;
        for (i = 0; i < 21 && e == 0; i++) {
