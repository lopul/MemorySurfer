...
static enum Block block_seq[P_HISTOGRAM+1][8] = {
	...
	{ B_START_HTML, B_HIDDEN_CAT, B_HIDDEN_ARRANGE, B_HIDDEN_CAT_NAME, B_HIDDEN_SEARCH_TXT, B_CLOSE_DIV, B_ASK, B_END }, // P_ASK
	{ B_START_HTML, B_HIDDEN_CAT, B_HIDDEN_ARRANGE, B_HIDDEN_CAT_NAME, B_HIDDEN_SEARCH_TXT, B_CLOSE_DIV, B_RATE, B_END }, // P_RATE
...
int multi_delim (struct Multi *mult)
{
	int e;
	int i;
	int ch;
						...
						if (e == 0) {
							...
							case F_LEARN_ACTION:
								if (memcmp (wms->mult.post_lp, "Learn", 5) == 0)
									...
									wms->seq = S_SHOW;
								else if (memcmp (wms->mult.post_lp, "Reveal", 6) == 0)
									wms->seq = S_SHOW;
								else if (memcmp (wms->mult.post_lp, "Stop", 4) == 0)
									wms->seq = S_SELECT_LEARN_CAT;
		...
		while ((bl = block_seq[wms->page][bl_i++]) != B_END && e == 0) {
			...
			case B_UPLOAD:
				...
				printf("Content-Type: text/html; charset=utf-8\n\n"
							 ...
							 "\t\t\t</div>\n"
							 "\t\t\t<p>Select a (previously exported .XML) File to upload (which should be used for the Import)</p>\n"
			...
			case B_SELECT_EDIT_CAT:
				assert(wms->file_title_str != NULL && strlen(wms->tok_str) == 40);
				printf("\t\t\t<p>Select a category to edit</p>\n");
				gen_html_cat (wms->ms.n_first, 3, H_CHILD, wms);
			...
			case B_SELECT_LEARN_CAT:
				assert(wms->file_title_str != NULL && strlen(wms->tok_str) == 40);
				printf("\t\t\t<p>Select a category to learn</p>\n");
				gen_html_cat(wms->ms.n_first, 3, H_CHILD, wms);
			...
			case B_SELECT_SEARCH_CAT:
				assert(wms->file_title_str != NULL && strlen(wms->tok_str) == 40);
				printf("\t\t\t<p>Select a category to search</p>\n");
				gen_html_cat (wms->ms.n_first, 3, H_CHILD, wms);
			...
			case B_PREFERENCES:
				...
				break;
			case B_ABOUT:
				printf("\t\t\t<p>MemorySurfer 1.0.0.1</p>\n"
							 ...
							 "but WITHOUT ANY WARRANTY; without even the implied warranty of\n"
							 "MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the\n"
							 "<a href=\"https://www.gnu.org/licenses/\">GNU General Public License</a> for more details.</cite>\n"
...
//						dis_str,
					...
					for (y = 0; y < 3; y++) {
						printf ("\t\t\t\t<tr>\n");
						for (x = 0; x < 2; x++)
							printf("\t\t\t\t\t<td><label><input type=\"radio\" disabled>Level</label></td>\n");
						printf("\t\t\t\t</tr>\n");
					}
					assert(wms->ms.cards_nel >= 0);
					printf("\t\t\t</table>\n"
								 "\t\t</form>\n"
								 "\t\t<code>%s; nel: %d</code>\n"
								 "\t</body>\n"
...
//									 "\t\t\t\t<input type=\"submit\" name=\"learn_action\" value=\"Reveal\" disabled>\n"
						...
						if (card_ptr->card_state == STATE_NEW) {
							time_diff = lvl_s[1];
							lvl = 0;
						}
						else {
							time_diff = wms->ms.timestamp - card_ptr->card_time;
							for (i = 0; i < 21; i++)
								if (lvl_s[i] >= card_ptr->card_strength)
									break;
							lvl = i;
						}
						j = lvl - 2;
...
int ms_get_card_sa (struct MemorySurfer *ms)
{
	int e;
	struct Card *card_ptr;
	...
	if (ms->card_i != -1)
	{
		...
		e = sa_load (&ms->card_sa, &ms->imf, card_ptr->card_qai);
	}
	return e;
}
...
int ms_close (struct MemorySurfer *ms)
{
	int e;
	assert (ms->imf_filename != NULL);
	e = imf_close (&ms->imf);
		...
		if (e == 0) {
			...
			if (e == 0) {
				...
				if (e == 0) {
					...
					if (e == 0) {
						...
						while ((act_c = action_seq[wms->seq][act_i++]) != A_END && e == 0)
						{
							...
							case A_LOGIN:
								wms->page = P_PASSWORD;
								wms->mode = wms->seq == S_GO_CHANGE ? M_PW_CHANGE : M_DEFAULT;
								break;
							case A_HISTOGRAM:
								for (i = 0; i < 100; i++)
									wms->hist_bucket[i] = 0;
						...
						if (e == 0)
							e = fclose(dbg_stream);
						if (e != 0)
							fprintf(stderr, "appending to log failed\n");
						e = saved_e;
					}
					else {
						wms->static_msg = "can't open debug.csv";
						wms->static_btn_main = "OK";
						wms->todo_main = S_NONE;
						wms->page = P_MSG;
					}
				...
				if (e != 0 && wms->page != P_MSG) {
					...
					wms->file_title_str = NULL;
					e2str(e, e_str);
					snprintf(msg, sizeof(msg), "Unexpected error (%s)", e_str);
					wms->static_msg = msg;
					wms->static_btn_main = "OK";
					wms->todo_main = S_NONE;
					wms->page = P_MSG;
				...
				if (e == 0 || wms->page == P_MSG) {
					e = gen_html(wms);
					if (e != 0)
						fprintf(stderr, "gen_html failed\n");
				}
			}
			else
