...
static const int32_t MS_VERSION = 0x010001a6;
    ...
    case 6:
      if (memcmp(mult->post_lp, "Reveal", 6) == 0) {
        ...
        if (wms->from_page == P_SELECT_DECK || wms->from_page == P_PREFERENCES) {
          ...
          if (wms->saved_mode != M_MSG_RESUME) {
            e = wms->todo_alt == -1;
            if (e == 0) {
