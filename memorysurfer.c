...
static const int32_t MSF_VERSION = 0x010001b1;
...
static enum Action action_seq[S_END+1][17] = {
  ...
  { A_SLASH, A_GATHER, A_OPEN, A_READ_PASSWD, A_AUTH_TOK, A_GEN_TOK, A_LOAD_CARDLIST_OLD, A_EDIT, A_GET_CARD, A_END }, // S_EDIT
  ...
  { A_GATHER, A_OPEN, A_READ_PASSWD, A_AUTH_TOK, A_GEN_TOK, A_RETRIEVE_MTIME, A_LOAD_CARDLIST_OLD, A_TEST_CARD, A_UPDATE_QA_OLD, A_EDIT, A_GET_CARD, A_SYNC, A_END }, // S_EDIT_SYNC
            ...
            if (e == 0) {
              ...
              while ((act_c = action_seq[wms->seq][act_i++]) != A_END && e == 0) {
                ...
                case A_LOAD_CARDLIST:
                  ...
                  data_size = imf_get_size(&wms->ms.imf, wms->ms.cat_t[wms->ms.cat_i].cat_cli);
                  assert(data_size >= 0 && wms->ms.card_l == NULL);
                  wms->ms.card_l = malloc(data_size);
                  e = wms->ms.card_l == NULL;
                  if (e == 0) {
                    e = imf_get(&wms->ms.imf, wms->ms.cat_t[wms->ms.cat_i].cat_cli, wms->ms.card_l);
                    if (e == 0) {
                      assert(wms->ms.card_a == 0);
